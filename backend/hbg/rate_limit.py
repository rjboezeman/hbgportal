...
from hbgapi.session.headers import getSIDHeader
...
limit_messages = [
    ...
    "Try to meditate once in a while..."
]
...
class RateLimiter(object):
    ...
    def rate_limit(self, f=None, limit=None, period=None):
        ...
        limit = limit if limit else app.config['RATELIMITE_LIMIT']
        period = period if period else app.config['RATELIMIT_PERIOD']
        ...
        def wrapped(*args, **kwargs):
            ...
            sessionId = getSIDHeader()
            if sessionId:
                #app.logger.debug(f"Rate limiting on sessionId: {sessionId}")
                key = f"{f.__name__}/{sessionId}"
            else:
                #app.logger.debug(f"Rate limiting on IP address: {request.remote_addr}")
            ...
            if not allowed:
                response = jsonify(
                    {   
                        ...
                        'message': random.choice(limit_messages)
                    })
